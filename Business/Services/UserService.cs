...
public class UserService(IUserRepository userRepository, UserManager<UserEntity> userManager, RoleManager<IdentityRole> roleManager) : IUserService
{
    ...
    /* Add New User kan också göras i Authentication-delen (i AuthServices) */
    public async Task<UserResult> CreateUserAsync(SignUpFormData formData, string roleName = "User")
    {
        ...
        //Control that Email does not already exist in database
        ...
        try
        {
            //Map from a SignUpFormData to a UserEntity
            ...
            if (result.Succeeded)
            {
                ...
                return result.Succeeded
                    ...
                    : new UserResult { Succeeded = true, StatusCode = 201, Error = "User created but role not added to user." };
            }
